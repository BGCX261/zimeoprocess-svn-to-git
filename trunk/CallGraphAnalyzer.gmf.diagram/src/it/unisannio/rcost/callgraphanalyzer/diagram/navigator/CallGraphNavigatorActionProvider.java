...
import it.unisannio.rcost.callgraphanalyzer.diagram.part.CallGraphDiagramEditor;
 ...
 * @generated
 */
public class CallGraphNavigatorActionProvider extends CommonActionProvider {
	...
	private class OpenDiagramAction extends Action {
		...
		public final void selectionChanged(IStructuredSelection selection) {
			...
			if (selection.size() == 1) {
				...
				if (selectedElement instanceof CallGraphNavigatorItem) {
					selectedElement = ((CallGraphNavigatorItem) selectedElement).getView();
				} else if (selectedElement instanceof IAdaptable) {
					selectedElement = ((IAdaptable) selectedElement).getAdapter(View.class);
				}
				if (selectedElement instanceof Diagram) {
					Diagram diagram = (Diagram) selectedElement;
					if (GraphEditPart.MODEL_ID
							.equals(CallGraphVisualIDRegistry.getModelID(diagram))) {
						myDiagram = diagram;
					}
				}
			}
		...
		public void run() {
			...
			try {
				page.openEditor(editorInput, CallGraphDiagramEditor.ID);
